...
class BooleanTermFunSuite extends FunSuite {
  ...
  test("boolean term must be isBoolean") {
     val t = TermWare.instance.booleanSignature.createConstant(false).get;
     var b = t.isBoolean;
     assert(t.isBoolean);
  }
  ...
  test("true and false term must not be unificated") {
     val t = TermWare.instance.booleanSignature.createConstant(true).get;
     val f = TermWare.instance.booleanSignature.createConstant(false).get;
